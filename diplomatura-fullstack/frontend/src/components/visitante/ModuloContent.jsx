...
import { Code, Image, Copy, Check, Video, Link as LinkIcon, ExternalLink, Eye, Award } from 'lucide-react';
...
export function ModuloContent({ tema, proyecto, tipoContenido = 'tema' }) {
  ...
  if (tipoContenido === 'proyecto' && proyecto) {
    return (
      <div className="h-full overflow-y-auto">
        <div className="p-6 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl mb-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Módulo {proyecto.modulo_id?.orden} - {proyecto.modulo_id?.nombre}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          <div>
            <h2 className="text-xl font-semibold mb-3">{proyecto.titulo}</h2>
          ...
          {proyecto.requisitos && (
            <div className="bg-muted/20 p-4 rounded-xl">
              ...
              <div className="prose prose-sm dark:prose-invert max-w-none">
                <p className="whitespace-pre-wrap text-sm">{proyecto.requisitos}</p>
              </div>
            </div>
          )}
  ...
  // Si es proyecto final pero no hay datos
  if (tipoContenido === 'proyecto' && !proyecto) {
    return (
  ...
  if (!tema) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground p-6">
        <p>Selecciona un módulo y tema para ver el contenido</p>
      </div>
    );
  }

  // Renderizar tema normal con ejemplos
  return (
    <div className="h-full overflow-y-auto">
      {/* Header del tema */}
      <div className="p-6 bg-muted/10 rounded-xl mb-4">
        <h1 className="text-2xl font-bold mb-2">{tema.titulo}</h1>
          ...
          {ejemplos.map((ejemplo) => {
            ...
            return (
              <div key={ejemplo._id} className="border border-border/50 rounded-xl overflow-hidden">
                <div 
                  className="p-4 bg-muted/10 cursor-pointer hover:bg-muted/20 transition-colors flex justify-between items-center"
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {hasCode && <span className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary">Código</span>}
                    {hasImages && <span className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary">Imágenes</span>}
                    {hasVideos && <span className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary">Videos</span>}
                    {hasRecursos && <span className="text-xs px-2 py-1 rounded-full bg-primary/10 text-primary">Recursos</span>}
                    <svg 
                  ...
                  <div className="p-4 space-y-4 border-t border-border/50">
                    ...
                    {ejemplo.explicacion && (
                      <div className="bg-muted/20 p-4 rounded-lg">
                        <p className="text-sm">{ejemplo.explicacion}</p>
                      </div>
                    )}
                        ...
                        {showPreview ? (
                          ...
                          <div>
                            <div className="flex justify-between items-center mb-2">
                              <h4 className="text-sm font-medium text-muted-foreground">Código HTML</h4>
                              <button
                            ...
                            <pre className="bg-muted/30 p-4 rounded-xl overflow-x-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {ejemplo.videos.map((video, i) => (
                            <div key={i} className="rounded-lg overflow-hidden">
                              ...
                              {video.titulo && (
                                <p className="text-xs text-muted-foreground mt-1">{video.titulo}</p>
                              )}
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="space-y-2">
                          {ejemplo.recursos.map((recurso, i) => (
                            ...
                            >
                              ...
                              <span className="text-xs text-muted-foreground">{recurso.tipo}</span>
                            </a>
                          ))}
                        </div>
                      </div>
